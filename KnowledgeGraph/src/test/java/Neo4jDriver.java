...
//                parameters( "name", "Arthur" ) )
        ...
        StatementResult result = session.run("MATCH p=(n:law)-[*1..3]->() where n.id='5a2e5745fd1c49151460fbfb' RETURN p LIMIT 25");
...
//        String id = "5a3b57eaed8e9d35307daaba";
        while (result.hasNext()) {
            ...
            Iterable<Relationship> rela = path.relationships();
            Iterator<Relationship> irela = rela.iterator();
            while (irela.hasNext()) {
                Relationship relationship = irela.next();
                System.out.println(relationship.type());
            }
//            System.out.println(path.start().get("name"));
//            System.out.println(path.end().get("name"));
            Iterable<Node> pinode = path.nodes();
            Iterator<Node> ireladd = pinode.iterator();
            while (ireladd.hasNext()) {
                Node node = ireladd.next();
